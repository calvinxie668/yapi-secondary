...
class projectController extends baseController {
  ...
  async get(ctx) {
    ...
    let catInst = yapi.getInst(interfaceCatModel);
    let socketCatInst = yapi.getInst(socketCatModel);
    let cat = await catInst.list(params.id);
    let socketCat = await socketCatInst.list(params.id);
    result.cat = cat;
    result.socketCat = socketCat;
  ...
  async del(ctx) {
    ...
    let socketInst = yapi.getInst(socketModel);
    let socketCatInst = yapi.getInst(socketCatModel);
    ...
    await socketInst.delByProjectId(id);
    await socketCatInst.delByProjectId(id);
    await cronInst.delByProjectId(id);
