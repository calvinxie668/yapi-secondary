...
@connect(
  state => {
    return {
      ...
      totalTableList: state.inter.totalTableList2,
...
class InterfaceList extends Component {
  ...
  handleRequest = async props => {
    ...
    if (!params.actionId) {
      ...
      await this.props.fetchSocketList(option);
      ...
      await this.props.fetchSocketCatList(option);
    }
  };
  ...
  handleChangeInterfaceCat = (desc, name) => {
    ...
    axios.post('/api/socket/up_cat', params).then(async res => {
      if (res.data.errcode !== 0) {
        return message.error(res.data.errmsg);
      }
      let project_id = this.props.match.params.id;
      await this.props.getProject(project_id);
      await this.props.fetchSocketListMenu(project_id);
  ...
  handleAddInterface = data => {
    ...
    axios.post('/api/socket/add', data).then(res => {
      ...
      this.props.history.push('/project/' + data.project_id + '/socket/api/' + interfaceId);
      this.props.fetchSocketListMenu(data.project_id);
    });
  };
  ...
  changeInterfaceCat = async (id, catid) => {
    ...
    if (result.data.errcode === 0) {
      ...
      this.handleRequest(this.props);
      this.props.fetchSocketListMenu(this.props.curProject._id);
    } else {
  ...
  render() {
    ...
    const columns = [
      {
        ...
        render: (text, item) => {
          return (
            <Link to={'/project/' + item.project_id + '/socket/api/' + item._id}>
    ...
    let intername = '',
      desc = '';
    let cat = this.props.curProject ? this.props.curProject.socketCat : [];
