...
class InterfaceMenu extends Component {
  static propTypes = {
    ...
    deleteSocketData: PropTypes.func,
    initInterface: PropTypes.func,
    history: PropTypes.object,
    router: PropTypes.object,
    getProject: PropTypes.func,
    fetchSocketCatList: PropTypes.func,
    fetchSocketList: PropTypes.func
  };
  ...
  async getList() {
    let r = await this.props.fetchSocketListMenu(this.props.projectId);
    this.setState({
  ...
  onSelect = selectedKeys => {
    ...
    let basepath = '/project/' + match.params.id + '/socket/api';
    if (curkey === 'root') {
  ...
  handleAddInterface = (data, cb) => {
    ...
    axios.post('/api/socket/add', data).then(res => {
      ...
      this.props.history.push('/project/' + this.props.projectId + '/socket/api/' + interfaceId);
      this.getList();
      this.setState({
        visible: false
      });
  ...
  handleAddInterfaceCat = data => {
    data.project_id = this.props.projectId;
    axios.post('/api/socket/add_cat', data).then(res => {
      if (res.data.errcode !== 0) {
        return message.error(res.data.errmsg);
      }
  ...
  handleChangeInterfaceCat = data => {
    ...
    axios.post('/api/socket/up_cat', params).then(res => {
      if (res.data.errcode !== 0) {
        return message.error(res.data.errmsg);
      }
  ...
  showConfirm = data => {
    ...
    const ref = confirm({
      ...
      cancelText: '取消',
      async onOk() {
        await that.props.deleteSocketData(id, that.props.projectId);
        await that.getList();
        await that.props.fetchSocketCatList({ catid });
        ref.destroy();
        that.props.history.push(
          '/project/' + that.props.match.params.id + '/socket/api/cat_' + catid
        );
      },
      onCancel() {
        ref.destroy();
  ...
  showDelCatConfirm = catid => {
    ...
    const ref = confirm({
      ...
      cancelText: '取消',
      async onOk() {
        await that.props.deleteSocketCatData(catid, that.props.projectId);
        await that.getList();
        // await that.props.getProject(that.props.projectId)
        await that.props.fetchSocketList({ project_id: that.props.projectId });
        that.props.history.push('/project/' + that.props.match.params.id + '/socket/api');
        ref.destroy();
      },
      onCancel() {}
    });
  ...
  copyInterface = async id => {
    let interfaceData = await this.props.fetchSocketData(id);
  ...
  onDrop = async e => {
    ...
    if (id.indexOf('cat') === -1) {
      ...
      this.props.fetchSocketList({ project_id: projectId });
      if (router && isNaN(router.params.actionId)) {
        // 更新分类list下的数据
        let catid = router.params.actionId.substr(4);
        this.props.fetchSocketCatList({ catid });
      }
    } else {
      // 分类之间拖动
    ...
    const searchBox = (
      <div className="interface-filter">
        ...
        {this.state.visible ? (
          ...
          >
            <AddInterfaceForm
              catdata={this.props.curProject.socketCat}
              ...
              <TreeNode
                ...
                title={
                  <Link
                    ...
                    to={'/project/' + matchParams.id + '/socket/api'}
                  >
                    ...
                    title={
                      ...
                      >
                        <Link
                          ...
                          to={'/project/' + matchParams.id + '/socket/api/cat_' + item._id}
                        >
